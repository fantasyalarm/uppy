...
const AWSProviderViews = require('./AWSProviderViews')
const { h } = require('preact')

module.exports = class AWSProvider extends Plugin {
  static VERSION = require('../package.json').version

  constructor (uppy, opts) {
    super(uppy, opts)
    this.id = this.opts.id || 'AWSProvider'
    this.title = this.opts.title || 'AWSProvider'
    Provider.initPlugin(this, opts)
    this.title = this.opts.title || 'AWSProvider'
    this.icon = () => (
      <svg width="32" height="32" viewBox="0 0 256 310" version="1.1">
        <g>
  ...
  install () {
    this.view = new AWSProviderViews(this, {
